...
int vitesseDelay = 800; // Vitesse (microsecondes)

void setup() {
  Serial.begin(115200);
  Serial.println("--- Test Clonage Logiciel (Y + X) ---");
  Serial.println("Tapez 'Y' pour faire avancer le portique.");
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (commande == 'Y' || commande == 'y') {
      Serial.println("Avance (1000 pas)...");
      bougerDeuxMoteurs(1000, HIGH); 
      
      delay(1000);
      
      Serial.println("Recule (1000 pas)...");
      bougerDeuxMoteurs(1000, LOW);
    }
  }
}
